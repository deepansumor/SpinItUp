...
export default class Slice {
    ...
    static create(segments) {
        console.log('Creating slices with segments:', segments);
    ...
    static getClickedSliceIndex(event, canvas, size, slices) {
        console.log('Determining clicked slice based on event:', event);
        ...
        if (distanceFromCenter > size / 2) {
            console.log('Click outside the wheel.');
        ...
        for (const slice of slices) {
            if (
                ...
                console.log(`Clicked slice index: ${slice.index}`);
                return slice.index;
            }
        }

        console.log('No slice found for the click event.');
        return null;
    }
